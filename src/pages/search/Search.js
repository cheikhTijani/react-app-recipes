    ...
    return (
        <div>
            ...
            {data && data.length === 0 && <div>
                <h3 style={{ color: (mode === 'dark' ? '#fff' : color) }} className='page-title'>No Results for your search: "{query}"</h3>
                <button className='back-btn' onClick={() => history.push('/')}>Go Back</button>
            </div>
            }
            {data && data.length !== 0 && <h3 style={{ color: (mode === 'dark' ? '#fff' : color) }} className='page-title'>Search Results for "{query}"</h3>}
